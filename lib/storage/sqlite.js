...
StorageSqlite.prototype.all = function all() {
  ...
  return new Promise(function(fulfill, reject) {
    self.db.serialize(function() {
      self.db.each("SELECT m.id, m.`from`, m.timestamp, m.subject, a.type as address_type, a.address, b.type as body_type, b.message as body_message  FROM messages m LEFT JOIN addresses a ON (m.id=a.id) LEFT JOIN bodies b ON (m.id=b.id) ORDER BY timestamp, m.id ",
