...
var search = function search (where, id, callback) {
  ...
  var safeDBEach = function (sql, idValue, cb) {
    if (idValue) {
      self.db.each(sql, idValue, function each (err, row) {
        if (err) { throw err; }
        handleRow(row);
      }, cb);
    } else {
    ...
    self.db.serialize(function () {
      search.apply(self, [null, null, function (messages) {
        if (self.options.messageLimit !== false) {
          return resolve(messages.slice(-self.options.messageLimit));
        } else {
...
StorageSqlite.prototype.delete = function (id) {
  var self = this;
  return new Promise(function (resolve, reject) {
    var functions = [];
    ['messages', 'addresses', 'bodies', 'raw'].forEach(function (table) {
...
StorageSqlite.prototype.deleteAll = function (id) {
  var self = this;
  return new Promise(function (resolve, reject) {
    var functions = [];
    ['messages', 'addresses', 'bodies', 'raw'].forEach(function (table) {
